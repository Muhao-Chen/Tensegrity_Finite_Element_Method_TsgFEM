...
E=Eb*ones(ne,1);
l0=kron(ones(6,1),[15;50;55.8;41.5;40.1;39.4;39.3;36.7;65.7;49;61.9]);  % rest length should be accurate
% l0=l;
rho=rho_b*ones(ne,1);
mass=A.*l0.*rho;
...
dnb_t([find(Ib_new(ind_dnb(7),:)),find(Ib_new(ind_dnb(8),:))],:)=l0(1)*[cos(linspace(0,-2*pi,substep)+theta1)-cos(theta1);sin(linspace(0,-2*pi,substep)+theta1)-sin(theta1)];
dnb_t([find(Ib_new(ind_dnb(9),:)),find(Ib_new(ind_dnb(10),:))],:)=l0(1)*[cos(linspace(0,-2*pi,substep)+theta2)-cos(theta1);sin(linspace(0,-2*pi,substep)+theta2)-sin(theta1)];
dnb_t([find(Ib_new(ind_dnb(11),:)),find(Ib_new(ind_dnb(12),:))],:)=l0(1)*[cos(linspace(0,-2*pi,substep)+theta3)-cos(theta1);sin(linspace(0,-2*pi,substep)+theta3)-sin(theta1)];
...
tenseg_video_slack(n_t,C_b,C_s,l0_t,index_s,[],[0,90],[],min(substep,50),name,savevideo,0);
