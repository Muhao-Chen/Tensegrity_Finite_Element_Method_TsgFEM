...
N=[   38.0000   38.0000   -8.7357  -39.6678         0  -19.4476   17.0047   30.3109
    ...
    0         0         0         0         0         0         0         0];
% the nodal coordinate is not necessary to be accurate like the following
% n1 = [38 7.8 0]';n2 = [38 22.8 0]'; n3 = [-20 40 0]'; n4 = [-40 -10 0]';
% n5=[0 0 0]'; n6=[-30 -50 0]'; n7=[10 -40 0]'; n8=[40 -70 0]';
% N = [n1 n2 n3 n4 n5 n6 n7 n8 ];

% Manually specify connectivity indices.
C_s_in=[];  % This is indicating that string connection
C_b_in = [1 2;2 3;3 4;3 5;4 5;4 6;5 7;6 7;6 8;7 8;2 7];  % Similarly, this is saying bar 1 connects node 1 to node 2,
...
E=Eb*ones(ne,1);
l0=[15;50;55.8;41.5;40.1;39.4;39.3;36.7;65.7;49;61.9];  % rest length should be accurate
% l0=l;
rho=rho_b*ones(ne,1);
mass=A.*l0.*rho;
...
% R3Ddata.Sradius=r_s;
R3Ddata.Nradius=2*max(R3Ddata.Bradius)*ones(nn,1);
tenseg_plot(N,C_b,C_s,[],[],[],'Jansen mechanism',R3Ddata);
...
ind_w=[];w=[];
ind_dnb=[2*3-2;2*3-1]; dnb0=[1;2];
ind_dl0=[]; dl0=[];
[w_t,dnb_t,l0_t,Ia_new,Ib_new]=tenseg_load_prestress(substep,ind_w,w,ind_dnb,dnb0,ind_dl0,dl0,l0,b,gravity,[0;9.8;0],C,mass);
theta=pi/2;
dnb_t(ind_dnb,:)=l0(1)*[cos(linspace(0,-2*pi,substep)+theta)-cos(theta);sin(linspace(0,-2*pi,substep)+theta)-sin(theta)];
...
%% input file of ANSYS
ansys_input_gp(N_out{1},C,A_gp,t_gp,b,Eb,Es,rho_b,rho_s,Gp,index_s,find(t_gp>0),fullfile(savePath,'Jansen_mechanism1'));
...
tenseg_video_slack(n_t,C_b,C_s,l0_t,index_s,[],[],[],min(substep,50),name,savevideo,0);
%% linearized dynaimcs
