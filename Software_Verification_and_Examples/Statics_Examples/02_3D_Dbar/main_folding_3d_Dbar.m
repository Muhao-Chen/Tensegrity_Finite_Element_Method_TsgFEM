...
%% input file of ANSYS
ansys_input_gp_00(N,C,A_gp,t_gp,b,Eb,Es,rho_b,rho_s,Gp,index_s,find(t_gp>0),ind_w,w,ind_dnb,dnb0,fullfile(savePath,'3d_Dbar_ansys'));
...
l_t=data_out.l_out;
%% plot member force
tenseg_plot_result(1:substep,t_t([1;7;10],:),{'bar','horizontal string','vertical string'},{'Load step','Force (N)'},fullfile(savePath,'plot_member_force.png'),saveimg);

%% plot member length
tenseg_plot_result(1:substep,l_t([1;7;10],:),{'bar','horizontal string','vertical string'},{'Load step','Length (m)'},fullfile(savePath,'plot_member_length.png'),saveimg);
...
%% save output data
if savedata==1
    save (fullfile(savePath,['3d_Dbar_',material{1},'.mat']));
end

%% make video of the dynamic
name=fullfile(savePath,['3d_Dbar','tf_',num2str(tf),material{1},num2str(material{2})]);
