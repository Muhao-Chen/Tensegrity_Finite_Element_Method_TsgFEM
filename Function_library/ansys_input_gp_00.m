function[]= ansys_input_gp_00(N,C,A_gp,t_gp,b,Eb,Es,rho_b,rho_s,Gp,index_s,index_s_gp,ind_w,w0,ind_dn,dn0,name)
...
fprintf(fid11,'\n');
%% second solve
% fprintf(fid11,'!第二次求解（荷载）\n/SOLU\nANTYPE,0 \nNLGEOM,ON!考虑大变形 \nSSTIF,ON	!考虑应力刚化 \nNSUBST,100	!设置荷载步的子步数 \nAUTOTS,ON	!自动荷载步 \nOUTRES,ALL,ALL 	!输出控制 \n');
fprintf(fid11,'!Second solve for external force\n/SOLU\nANTYPE,0 \nNLGEOM,ON!consider large deformation \nSSTIF,ON	!prestress stiffness \nNSUBST,100	!Automatic time stepping \nAUTOTS,ON	!Automatic time stepping \nOUTRES,ALL,ALL 	!Output result \n');
fprintf(fid11,'\n');

%% force and constraints
for i=1:numel(ind_w)
    fprintf(fid11,'FK,%d,%c,%f\n',ceil(ind_w(i)/3),char(88+mod(ind_w(i)+2,3)),w0(i));
end
for i=1:numel(ind_dn)
    fprintf(fid11,'DK,%d,U%c,%f\n',ceil(ind_dn(i)/3),char(88+mod(ind_dn(i)+2,3)),dn0(i));
end
%% prestress and solve
% fprintf(fid11,'*DO,J,1,%d	!数据填入预应力数组\n	INISTATE,DEFINE,J,,,,PRESTRESS(J)\n*ENDDO\n',ne);
fprintf(fid11,'*DO,J,1,%d	!Prestress in initial state\n	INISTATE,DEFINE,J,,,,PRESTRESS(J)\n*ENDDO\n',ne);
