...
% E-strain
figure
xi = linspace(2*min(data_b_strain),2*max(data_b_strain),100000);
yi = interp1(data_b_strain, data_b_E, xi,'previous',0);
plot(data_b_strain, data_b_E,'ko', xi, yi,'linewidth',1.5);
axis([-0.025,0.025,-1.2*max(data_b_E),1.2*max(data_b_E)]);
xlabel('Strain','fontsize',14);
...
figure
xj = linspace(-1,2*max(data_s_strain),10000);
yj = interp1(data_s_strain, data_s_E, xj,'previous',0);
plot(data_s_strain, data_s_E,'k*', xj, yj,'linewidth',1.5);
axis([-0.015,0.025,-max(data_s_E),max(data_s_E)])
xlabel('Strain','fontsize',14);
